...
public sealed class GaussDBParameterCollection : DbParameterCollection, IList<GaussDBParameter>
{
    ...
    public new GaussDBParameter this[string parameterName]
    {
        ...
        set
        {
            ...
            if (!string.Equals(parameterName, value.TrimmedName, StringComparison.OrdinalIgnoreCase))
                throw new ArgumentException("Parameter name must be a case-insensitive match with the property 'ParameterName' on the given GaussDBParameter", nameof(parameterName));
    ...
    /// <returns>The index of the new <see cref="GaussDBParameter"/> object.</returns>
