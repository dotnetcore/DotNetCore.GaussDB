...
public class GaussDBCommand : DbCommand, ICloneable, IComponent
{
    ...
    /// <summary>
    /// Whether this <see cref="GaussDBCommand" /> is wrapped by an <see cref="GaussDBBatch" />.
    /// </summary>
    ...
    /// </summary>
    internal GaussDBCommand(int batchCommandCapacity, GaussDBConnection? connection = null)
    {
        GC.SuppressFinalize(this);
    ...
    /// Used when this <see cref="GaussDBCommand"/> instance is wrapped inside an <see cref="GaussDBBatch"/>.
    /// </summary>
    internal GaussDBCommand(GaussDBConnector connector, int batchCommandCapacity)
    ...
    public new GaussDBParameterCollection Parameters => _parameters ??= new();
...
";
                ...
                foreach (var batchCommand in InternalBatchCommands)
                {
                    ...
                    if (batchCommand.PositionalParameters.Count != paramTypeOIDs.Count)
                    {
                        ...
                        throw new GaussDBException(
                            "There was a mismatch in the number of derived parameters between the GaussDB SQL parser and the PostgreSQL parser. Please report this as bug to the GaussDB developers (https://github.com/GaussDB/GaussDB/issues).");
                    }
    ...
    async Task SendDeriveParameters(GaussDBConnector connector, bool async, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < InternalBatchCommands.Count; i++)
        {
            ...
            await connector.WriteParse(batchCommand.FinalCommandText!, Array.Empty<byte>(), GaussDBBatchCommand.EmptyParameters, async, cancellationToken).ConfigureAwait(false);
    ...
    internal void TraceCommandStart(GaussDBConnector connector)
    {
        Debug.Assert(CurrentActivity is null);
        if (GaussDBActivitySource.IsEnabled)
            CurrentActivity = GaussDBActivitySource.CommandStart(connector, CommandText, CommandType);
    }
    ...
    internal void TraceReceivedFirstResponse()
    {
        ...
        {
            GaussDBActivitySource.ReceivedFirstResponse(CurrentActivity);
        }
    }
    ...
    internal void LogExecutingCompleted(GaussDBConnector connector, bool executing)
    {
        ...
        object[] ParametersDbNullAsString(GaussDBBatchCommand c)
        {
