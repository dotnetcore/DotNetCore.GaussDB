...
public sealed class GaussDBBinaryImporter : ICancelable
{
    ...
    internal async Task Init(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
    {
        ...
        _params = new GaussDBParameter[NumColumns];
        _rowsImported = 0;
        _buf.StartCopyMode();
        WriteHeader();
    ...
    Task Write<T>(bool async, T value, CancellationToken cancellationToken = default)
    {
        ...
        if (p is not GaussDBParameter<T> typedParam)
            typedParam = new GaussDBParameter<T>();
        ...
        if (p is not null && (typeof(T) == typeof(object) || p._GaussDBDbType is not null || p._dataTypeName is not null))
        {
    ...
    public void Write<T>(T value, GaussDBDbType GaussDBDbType) =>
        Write(async: false, value, GaussDBDbType).GetAwaiter().GetResult();
    ...
    public Task WriteAsync<T>(T value, GaussDBDbType GaussDBDbType, CancellationToken cancellationToken = default)
        => Write(async: true, value, GaussDBDbType, cancellationToken);

    Task Write<T>(bool async, T value, GaussDBDbType GaussDBDbType, CancellationToken cancellationToken = default)
    {
        CheckColumnIndex();
        if (cancellationToken.IsCancellationRequested)
            return Task.FromCanceled(cancellationToken);
        ...
        ref var p = ref _params[_column];
        if (p is not GaussDBParameter<T> typedParam)
            typedParam = new GaussDBParameter<T> { GaussDBDbType = GaussDBDbType };

        // We only report previous values if anything actually changed, this saves some checks during the write.
        PgTypeInfo? previousTypeInfo = null;
        PgConverter? previousConverter = null;
        PgTypeId previousTypeId = default;
        if (p is not null && (p._GaussDBDbType != GaussDBDbType || p._dataTypeName is not null))
        {
            p.GetResolutionInfo(out previousTypeInfo, out previousConverter, out previousTypeId);
            if (ReferenceEquals(p, typedParam))
            {
                p.ResetDbType();
                p.GaussDBDbType = GaussDBDbType;
            }
        }
    ...
    Task Write<T>(bool async, T value, string dataTypeName, CancellationToken cancellationToken = default)
    {
        ...
        ref var p = ref _params[_column];
        if (p is not GaussDBParameter<T> typedParam)
            typedParam = new GaussDBParameter<T> { DataTypeName = dataTypeName };

        // We only report previous values if anything actually changed, this saves some checks during the write.
        PgTypeInfo? previousTypeInfo = null;
        PgConverter? previousConverter = null;
        PgTypeId previousTypeId = default;
        if (p is not null && (p._GaussDBDbType is not null || p._dataTypeName != dataTypeName))
        {
            p.GetResolutionInfo(out previousTypeInfo, out previousConverter, out previousTypeId);
            if (ReferenceEquals(p, typedParam))
            {
    ...
    async Task Write<T>(bool async, T value, GaussDBParameter<T> param, PgTypeInfo? previousTypeInfo, PgConverter? previousConverter, PgTypeId previousTypeId, CancellationToken cancellationToken = default)
    {
        CheckReady();
        if (_column == -1)
