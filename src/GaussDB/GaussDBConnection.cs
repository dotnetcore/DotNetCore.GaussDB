...
public sealed class GaussDBConnection : DbConnection, ICloneable, IComponent
{
...
#else
    /// <summary>
    /// Creates and returns a <see cref="GaussDBBatch"/> object associated with the <see cref="GaussDBConnection"/>.
    /// </summary>
    /// <returns>A <see cref="GaussDBBatch"/> object.</returns>
    public GaussDBBatch CreateBatch() => new(this);
#endif
    ...
    {
        if (_disposed)
            ThrowHelper.ThrowObjectDisposedException(nameof(GaussDBConnection));
    }
...
#endif
    {
        return GaussDBSchema.GetSchema(async: true, this, collectionName, restrictions, cancellationToken);
    }
...
readonly struct EndScopeDisposable : IDisposable
{
    ...
    public EndScopeDisposable(GaussDBConnection connection) => _connection = connection;
