...
public sealed class GaussDBBinaryExporter : ICancelable
{
    ...
    /// <returns>The value of the column</returns>
    public T Read<T>(GaussDBDbType type) => Read<T>(async: false, type, CancellationToken.None).GetAwaiter().GetResult();
    ...
    async ValueTask<T> Read<T>(bool async, GaussDBDbType? type, CancellationToken cancellationToken)
    {
        ThrowIfNotOnRow();
        ...
        PgConverterInfo GetInfo(GaussDBDbType? type, out bool asObject)
        {
