...
sealed class AuthenticationSASLMessage : AuthenticationRequestMessage
{
    ...
    internal AuthenticationSASLMessage(GaussDBReadBuffer buf)
    {
        if (buf == null)
        {
            throw new GaussDBException("Received AuthenticationSASL message is null!");
        }
    ...
    {
        var pwdtype = (GaussDBAuthenticationPasswordType)buf.ReadInt32();

        return pwdtype switch
        {
            GaussDBAuthenticationPasswordType.MD5_PASSWORD => AuthenticationMD5PasswordMessage.Load(buf),
            GaussDBAuthenticationPasswordType.SHA256_PASSWORD => new AuthenticationSASLMessage(buf),
            _ => throw new GaussDBException("Received AuthenticationSASL message cannot recognize the password authentication type.")
        };
    }
}
...
sealed class AuthenticationSASLContinueMessage : AuthenticationRequestMessage
{
    ...
    internal AuthenticationSASLContinueMessage(GaussDBReadBuffer buf, int len)
    {
...
sealed class AuthenticationSCRAMServerFirstMessage
{
    ...
    {
        var data = GaussDBWriteBuffer.UTF8Encoding.GetString(bytes);
        ...
        if (nonce == null)
            throw new GaussDBException("Server nonce not received in SCRAM server-first message");
        if (salt == null)
            throw new GaussDBException("Server salt not received in SCRAM server-first message");
        if (iteration == -1)
            throw new GaussDBException("Server iterations not received in SCRAM server-first message");
...
sealed class AuthenticationSASLFinalMessage : AuthenticationRequestMessage
{
    ...
    internal AuthenticationSASLFinalMessage(GaussDBReadBuffer buf, int len)
    {
        Payload = new byte[len];
        buf.ReadBytes(Payload, 0, len);
    }
...
sealed class AuthenticationSCRAMServerFinalMessage
{
    ...
    internal static AuthenticationSCRAMServerFinalMessage Load(byte[] bytes, ILogger connectionLogger)
    {
        ...
        if (serverSignature == null)
            throw new GaussDBException("Server signature not received in SCRAM server-final message");
