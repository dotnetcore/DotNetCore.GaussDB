...
#pragma warning restore CS0618
    ...
    async Task RawOpen(SslMode sslMode, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken, bool isFirstAttempt = true)
    {
        try
        {
            if (async)
    ...
    internal async Task NegotiateEncryption(SslMode sslMode, GaussDBTimeout timeout, bool async, bool isFirstAttempt)
    {
        ...
        if (certPath != null)
        {
            ...
            if (Path.GetExtension(certPath).ToUpperInvariant() != ".PFX")
            {
                ...
                // This is a breaking change for .NET 5 as of GaussDB 8!
    ...
    async Task ConnectAsync(GaussDBTimeout timeout, CancellationToken cancellationToken)
    {
        ...
        if (timeout.IsSet)
        {
            ...
            perIpTimeout = new GaussDBTimeout(perIpTimespan);
        }
    ...
    void SetSocketOptions(Socket socket)
    {
        ...
        if (Settings.TcpKeepAliveTime > 0)
        {
            ...
            if (result != 0)
                throw new GaussDBException($"Got non-zero value when trying to set TCP keepalive: {result}");
#else
            socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true);
        ...
        bool attemptPgCancellation = true)
    {
        ...
        return _cancellationTokenRegistration =
            cancellationToken.Register(static c => ((GaussDBConnector)c!).PerformUserCancellation(), this);
    }
        ...
        bool attemptPgCancellation = true)
    {
        ...
        var registration = cancellationToken.Register(static c => ((GaussDBConnector)c!).PerformUserCancellation(), this);
    ...
    internal readonly struct NestedCancellableScope : IDisposable
    {
        ...
        public NestedCancellableScope(GaussDBConnector connector, CancellationTokenRegistration registration, CancellationToken previousCancellationToken, bool previousAttemptPostgresCancellation)
        {
    ...
    internal async Task CloseOngoingOperations(bool async)
    {
        ...
        else if (copyOperation != null)
        {
            ...
            if (copyOperation is GaussDBBinaryImporter ||
                ...
                copyOperation is GaussDBRawCopyStream rawCopyStream && rawCopyStream.CanWrite)
            {
                try
                {
                    if (async)
        ...
        lock (SyncObj)
        {
            if (IsReady)
            {
                ...
                try
                {
                    ...
                    // see https://github.com/GaussDB/GaussDB/issues/3592
                    WriteBuffer.Clear();
                    WriteTerminate();
                    Flush();
                }
            ...
            lock (CancelLock)
            {
                ...
                if (connection is not null)
                {
                    ...
                    if (Settings.ReplicationMode == ReplicationMode.Off)
                    {
                        ...
                        // (see https://github.com/GaussDB/GaussDB/issues/3705#issuecomment-839908772)
                        Connection = null;
    ...
    internal async Task<bool> Wait(bool async, int timeout, CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                catch (Exception e) when (e is OperationCanceledException || e is GaussDBException npgEx && npgEx.InnerException is TimeoutException)
                {
