...
partial class GaussDBConnector
{
    ...
    internal async Task AuthenticateGSS(bool async)
    {
        ...
        throw new GaussDBException("NegotiateStream.AuthenticateAsClient completed unexpectedly without signaling success");
    }
    ...
    sealed class GSSPasswordMessageStream : Stream
    {
        ...
        internal GSSPasswordMessageStream(GaussDBConnector connector)
        ...
        async Task Write(byte[] buffer, int offset, int count, bool async, CancellationToken cancellationToken = default)
        {
            ...
            if (count > _leftToWrite)
                throw new GaussDBException($"NegotiateStream trying to write {count} bytes but according to frame header we only have {_leftToWrite} left!");
        ...
        async Task<int> Read(byte[] buffer, int offset, int count, bool async, CancellationToken cancellationToken = default)
        {
            ...
            if (count > _leftToRead)
                throw new GaussDBException($"NegotiateStream trying to read {count} bytes but according to frame header we only have {_leftToRead} left!");
