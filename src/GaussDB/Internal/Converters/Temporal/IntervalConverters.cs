    ...
    protected override TimeSpan ReadCore(PgReader reader)
    {
        ...
        return months > 0
            ? throw new InvalidCastException(
                "Cannot read interval values with non-zero months as TimeSpan, since that type doesn't support months. Consider using NodaTime Period which better corresponds to PostgreSQL interval, or read the value as GaussDBInterval, or transform the interval to not contain months or years in PostgreSQL before reading it.")
...
sealed class GaussDBIntervalConverter : PgBufferedConverter<GaussDBInterval>
{
    public override bool CanConvert(DataFormat format, out BufferRequirements bufferRequirements)
    {
        bufferRequirements = BufferRequirements.CreateFixedSize(sizeof(long) + sizeof(int) + sizeof(int));
    ...
    protected override GaussDBInterval ReadCore(PgReader reader)
    {
        var ticks = reader.ReadInt64();
        var day = reader.ReadInt32();
        var month = reader.ReadInt32();
        return new GaussDBInterval(month, day, ticks);
    }

    protected override void WriteCore(PgWriter writer, GaussDBInterval value)
    {
