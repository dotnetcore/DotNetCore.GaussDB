...
sealed class DateTimeConverterResolver<T> : PgConverterResolver<T>
{
    ...
    public PgConverterResolution? Get(DateTime value, PgTypeId? expectedPgTypeId, bool validateOnly = false)
    {
        if (value.Kind is DateTimeKind.Utc)
        {
            ...
            return expectedPgTypeId == _timestamp
                ? throw new ArgumentException(
                    string.Format(GaussDBStrings.TimestampNoDateTimeUtc, _options.GetDataTypeName(_timestamp).DisplayName, _options.GetDataTypeName(_timestampTz).DisplayName), nameof(value))
        ...
        {
            throw new ArgumentException(
                string.Format(GaussDBStrings.TimestampTzNoDateTimeUnspecified, value.Kind, _options.GetDataTypeName(_timestampTz).DisplayName), nameof(value));
        }
...
sealed class DateTimeConverterResolver
{
    ...
    public static DateTimeConverterResolver<GaussDBRange<DateTime>> CreateRangeResolver(PgSerializerOptions options, PgTypeId timestampTz, PgTypeId timestamp, bool dateTimeInfinityConversions)
    ...
    {
        if (typeof(TElement) != typeof(GaussDBRange<DateTime>))
        ...
        return new DateTimeConverterResolver<T>(options, static (resolver, value, expectedPgTypeId) =>
        {
            ...
            foreach (var element in (IList<GaussDBRange<DateTime>>)value)
            {
