...
sealed class RangeConverter<TSubtype> : PgStreamingConverter<GaussDBRange<TSubtype>>
{
    ...
    public override GaussDBRange<TSubtype> Read(PgReader reader)
        => Read(async: false, reader, CancellationToken.None).GetAwaiter().GetResult();

    public override ValueTask<GaussDBRange<TSubtype>> ReadAsync(PgReader reader, CancellationToken cancellationToken = default)
        => Read(async: true, reader, cancellationToken);

    async ValueTask<GaussDBRange<TSubtype>> Read(bool async, PgReader reader, CancellationToken cancellationToken)
    {
        ...
        if ((flags & RangeFlags.Empty) != 0)
            return GaussDBRange<TSubtype>.Empty;
        ...
        return new GaussDBRange<TSubtype>(lowerBound, upperBound, flags);
    }

    public override Size GetSize(SizeContext context, GaussDBRange<TSubtype> value, ref object? writeState)
    {
    ...
    public override void Write(PgWriter writer, GaussDBRange<TSubtype> value)
        => Write(async: false, writer, value, CancellationToken.None).GetAwaiter().GetResult();

    public override ValueTask WriteAsync(PgWriter writer, GaussDBRange<TSubtype> value, CancellationToken cancellationToken = default)
        => Write(async: true, writer, value, cancellationToken);

    async ValueTask Write(bool async, PgWriter writer, GaussDBRange<TSubtype> value, CancellationToken cancellationToken)
    {
